...
enemy_x_pos = random.randint(0, screen_width - enemy_width) # X좌표는 랜덤으로 설정
enemy_y_pos = 0 # Y좌표는 0으로 설정
enemy_speed = 10 # 적의 속도#

class Enemy:
    def __init__(self, x, y, speed):
        self.image = pygame.image.load("D:\\pythongame\\pygame_basic\\enemy.png")
...
#     
    ...
    # enemy_y_pos += enemy_speed # 적의 y좌표를 enemy_speed만큼 이동
...
#########################################################################################################################
    ...
    if enemy_y_pos > screen_height: # 적이 화면 아래로 벗어나면
        enemy_y_pos = 0 # 적의 y좌표를 0으로 설정
        enemy_x_pos = random.randint(0, screen_width - enemy_width) # 적의 x좌표를 랜덤으로 설정
    elif character_rect.colliderect(enemy_rect): # 캐릭터와 적이 충돌했는지 확인
        print("적과 충돌 했습니다. 게임을 종료합니다.")
        running = False # 게임 종료
    else:
